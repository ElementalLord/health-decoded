...
const dueAt = (daysAgo, now = at("2026-08-10")) =>
...
test("all approved Explain It Back challenges are reviewable without new medical content", () => {
  ...
  assert.deepEqual(
    spacedReviewConcepts.map(({ challengeId }) => challengeId),
    explainItBackChallenges.map(({ id }) => id),
  );
  assert.deepEqual(
    spacedReviewConcepts
      .filter(({ learnedFromLessonIds }) => !learnedFromLessonIds.length)
      .map(({ challengeId }) => challengeId),
    ["serving-size", "total-vs-added-sugars"],
  );
  ...
  assert.equal(
    spacedReviewConcepts
      .flatMap(({ learnedFromLessonIds }) => learnedFromLessonIds)
      .every((id) => lessonIds.has(id)),
    true,
  );
  assert.equal(spacedReviewConfig.initialIntervalDays > 0, true);
  assert.equal(
    spacedReviewConfig.successIntervalsDays.every((days) => days > 0),
    true,
  );
});
...
test("linked lesson exposure initializes the first due date three days after earliest learning", () => {
  ...
  assert.equal(
    spacedReviewConcepts.find(({ challengeId }) => challengeId === "a1c").learnedFromLessonIds
      .length,
    1,
  );
  assert.equal(
    spacedReviewConcepts.find(({ challengeId }) => challengeId === "serving-size")
      .learnedFromLessonIds.length,
    0,
  );
});
...
test("retry success, almost there, try again, and example viewing use gentle reinforcement intervals", () => {
  ...
  const days = (verdict, hadRetry, exampleViewed) =>
    (Date.parse(scheduleReviewResult({ state, verdict, now, hadRetry, exampleViewed }).nextDueAt) -
      now.getTime()) /
    86_400_000;
  ...
  assert.equal(
    scheduleReviewResult({ state, verdict: "got_it", now, hadRetry: true, exampleViewed: false })
...
test("a due weak concept outranks a stronger concept due five days earlier", () => {
  const now = at("2026-08-10");
  const result = selectReviewCandidate({
    now,
    manual: false,
    candidates: [
      candidate({ challengeId: "a1c", nextDueAt: dueAt(5), lastVerdict: "got_it" }),
      candidate({
        challengeId: "insulin-resistance",
        title: "Insulin resistance",
        nextDueAt: dueAt(0),
        lastVerdict: "try_again",
      }),
    ],
  });
...
test("a severely overdue concept outranks a slightly overdue equally strong concept", () => {
  ...
  const result = selectReviewCandidate({
    ...
    manual: false,
    candidates: [
      candidate({ challengeId: "a1c", nextDueAt: dueAt(20), lastVerdict: "got_it" }),
      candidate({
        challengeId: "insulin",
        title: "Insulin",
        nextDueAt: dueAt(1),
...
test("Foundations is only the final tie-breaker", () => {
  ...
  assert.equal(
    selectReviewCandidate({ candidates: [food, foundation], now, manual: false }).candidate
      ?.challengeId,
    "insulin",
  );
  assert.equal(
    selectReviewCandidate({
      candidates: [food, candidate({ ...foundation, nextDueAt: dueAt(1) })],
      now,
      manual: false,
...
test("manual review chooses one closest eligible concept and has an intentional empty result", () => {
  const now = at("2026-08-10");
  const future = [
    candidate({ challengeId: "a1c", nextDueAt: "2026-08-20T00:00:00.000Z" }),
    candidate({ challengeId: "insulin", title: "Insulin", nextDueAt: "2026-08-12T00:00:00.000Z" }),
  ];
  assert.equal(
    selectReviewCandidate({ candidates: future, now, manual: true }).candidate?.challengeId,
    "insulin",
  );
...
test("automatic prompts require a due concept and respect gap, weekly cap, and snooze", () => {
  ...
  assert.equal(
    shouldOfferAutomaticReview({ ...base, selection: { ...selection, due: false } }),
    false,
  );
  assert.equal(
    shouldOfferAutomaticReview({ ...base, lastPromptedAt: "2026-08-09T12:00:00.000Z" }),
    false,
  );
  assert.equal(
    shouldOfferAutomaticReview({
  ...
  assert.equal(
    shouldOfferAutomaticReview({ ...base, dismissedUntil: "2026-08-11T00:00:00.000Z" }),
    false,
  );
  assert.equal(
    selectReviewCandidate({ candidates: [selection.candidate], now, manual: true }).candidate
      ?.challengeId,
    "a1c",
    "manual review remains available while prompts are snoozed",
  );
});
...
test("persistence is metadata-only, idempotent, server-timed, and owner-scoped", async () => {
  const migration = await readFile(
    new URL("../supabase/migrations/20260810000003_spaced_review.sql", import.meta.url),
    "utf8",
  );
  ...
  assert.doesNotMatch(
    migration,
    /explanation\s+pg_catalog|response_text|prompt_text|medical_data/i,
  );
...
test("review mode reuses Explain It Back and never exposes a topic picker or changes streaks", async () => {
  const [component, api, journey, streakTypes, milestones] = await Promise.all([
    readFile(
      new URL(
        "../features/explain-it-back/components/explain-it-back-experience.tsx",
        import.meta.url,
      ),
      "utf8",
    ),
    ...
    readFile(
      new URL("../features/achievements/content/milestone-definitions.ts", import.meta.url),
      "utf8",
    ),
  ]);
  ...
  assert.ok(
    api.indexOf("if (!result.ok)") < api.indexOf("reviewRecorded = await recordSpacedReviewResult"),
    "evaluator failure returns before scheduling",
  );
  ...
  assert.match(milestones, /MILESTONE-MEMORY-IN-MOTION/);
  assert.doesNotMatch(milestones, /review streak|memory strength/i);
});
...
test("unknown challenges and malformed spaced-review requests are rejected", () => {
  assert.equal(
    explainItBackRequestSchema.safeParse({
      challengeId: "unknown",
      explanation: "This is a useful explanation.",
    }).success,
    false,
  );
  assert.equal(
...
test("Spaced Review is optional in Next Step and never replaces required learning", () => {
  const required = recommendNextStep({
    completedLessonCount: 8,
    currentLesson: { dayNumber: 9, estimatedMinutes: 8, status: "not_started", title: "Lesson" },
  ...
  const complete = recommendNextStep({
    completedLessonCount: 14,
    currentLesson: null,
    earnedMilestoneIds: new Set(["MILESTONE-MYTH-CHECKER"]),
    lastDismissed: null,
    today: "2026-08-10",
    hasDueSpacedReview: true,
  });
...
test("automatic prompt integration exists only on Journey and protected content remains untouched", async () => {
  const protectedRoots = [
    "features/lessons/content",
    "features/caregiver/content",
    "features/stories/content",
    "content/resources",
  ];
  for (const root of protectedRoots) {
    let names = [];
    try {
      names = await readdir(new URL(`../${root}/`, import.meta.url), { recursive: true });
    } catch {
      continue;
    }
  ...
  const promptComponent = await readFile(
    new URL("../features/spaced-review/components/journey-spaced-review.tsx", import.meta.url),
    "utf8",
  );

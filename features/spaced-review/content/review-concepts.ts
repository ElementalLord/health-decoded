...
export const spacedReviewConcepts = explainItBackChallenges.map((challenge) => {
  const concept = reviewConceptRegistry.find((entry) =>
    entry.explainItBackIds?.includes(challenge.id),
  );
  return {
...
export const spacedReviewConceptByChallengeId = new Map<
  string,
  (typeof spacedReviewConcepts)[number]

...
const reviewEventSchema = z
  .object({ challengeId: z.string().min(1).max(80), token: z.uuid() })
  .strict();
...
export async function recordSpacedReviewExampleAction(input: unknown) {
  ...
  return {
    ok: event ? await recordSpacedReviewExample(event.challengeId, event.token) : false,
  } as const;
}
...
export async function recordSpacedReviewPromptAction(input: unknown) {
  const event = parseEvent(input);
  return {
    ok: event ? await recordSpacedReviewPrompt(event.challengeId, event.token) : false,
  } as const;
}

export async function snoozeSpacedReviewPromptsAction(input: unknown) {
  const parsed = z
    .object({ challengeId: z.string().min(1).max(80) })
    .strict()
    .safeParse(input);
  return {
    ok:
      parsed.success && isReviewableChallengeId(parsed.data.challengeId)
        ? await snoozeSpacedReviewPrompts(parsed.data.challengeId)
        : false,
  } as const;
}

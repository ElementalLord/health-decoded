...
export async function saveLessonPositionAction(input: unknown): Promise<SaveLessonPositionResult> {
  ...
  const response = await settleOptional(
    async () => {
      ...
      return database.rpc("save_lesson_block_position", {
        p_block_index: parsed.data.blockIndex,
        p_lesson_progress_id: parsed.data.lessonProgressId,
      });
    },
    null,
    () => logger.error("lesson_reader.position_save_rejected"),
  );
  if (!response) return { ok: false, message: "Your place could not be saved right now." };

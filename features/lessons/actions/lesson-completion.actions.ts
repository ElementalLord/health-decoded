...
export async function completeLessonAction(input: unknown): Promise<CompleteLessonActionResult> {
  ...
  const reflection = parsed.data.reflection;
  if (reflection) {
    const reflectionSaved = await settleOptional(
      async () => {
      ...
      false,
      () => logger.error("lesson_reflection.write_rejected"),
    );
  ...
  if (completed.data.firstTimeCompletion) {
    await settleOptional(
      () => recognizeMilestoneEvent({ event: "lesson_completed" }, { recordStreak: true }),
      null,
      () => logger.error("lesson_completion.optional_recognition_rejected"),
    );
  }

...
> {
  ...
  if (!user.ok || !user.data.email) return err(user.ok ? unexpectedError() : user.error);
  ...
  if (profileResult.error || !profileResult.data || settingsResult.error)
    return err(unexpectedError());
  let settings = settingsResult.data;
  if (!settings) {
    const created = await database
      .from("user_settings")
      .upsert({ user_id: user.data.id, ...defaults })
      .select("reduced_motion, preferred_text_scale, locale, timezone")
      .maybeSingle();
    if (created.error || !created.data) return err(unexpectedError());

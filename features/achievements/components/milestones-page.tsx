...
function MilestoneItem({
  ...
  return (
    <li
      className={`${earned ? styles.earned : styles.available} ${featured ? styles.featuredItem : ""}`}
    ...
    >
      ...
      <div>
        ...
        <small>
          <strong>Recognized when:</strong> {definition.criteriaLabel}
        </small>
      </div>
    </li>
  );
...
export function MilestonesPage({ earned }: { earned: readonly EarnedMilestone[] }) {
  ...
  const categories = Object.keys(milestoneCategoryLabels) as Array<
    keyof typeof milestoneCategoryLabels
  >;
  return (
    <main className={styles.page}>
      <header className={styles.hero}>
        ...
        <p className={styles.boundary}>
          {
            "Milestones recognize learning and preparation inside Health Decoded. They do not measure your health, treatment success, or quality of diabetes management."
          }
        </p>
      </header>
      ...
      <div className={styles.categories}>
        {categories.map((category) => {
          const entries = milestoneDefinitions.filter(
          ...
          return (
            <section aria-labelledby={`milestone-category-${category}`} key={category}>
              <div className={styles.categoryHeading}>
                ...
                <p>
                  {entries.filter((definition) => earnedById.has(definition.id)).length} of{" "}
                  {entries.length} recognized
                </p>
              </div>
              <ul>

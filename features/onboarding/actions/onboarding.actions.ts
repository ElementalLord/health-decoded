...
export async function completeOnboardingAction(_: OnboardingFormState, formData: FormData): Promise<OnboardingFormState> {
  const parsed = onboardingSchema.safeParse(Object.fromEntries(formData.entries()));
  if (!parsed.success) return { status: "error", message: parsed.error.issues[0]?.message ?? "Check your setup and try again." };
  const result = await completeOnboarding(parsed.data);
  if (!result.ok) return { status: "error", message: "We could not save your setup right now. Please try again." };
  redirect("/account");
}

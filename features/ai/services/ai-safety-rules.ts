...
const injectionPattern =
  /\b(ignore|forget|disregard|override|bypass|replace|disable|evade)\b.{0,100}\b(previous|prior|system|developer|health decoded|safety|instruction|guardrail|rule|source|citation)s?\b|\b(?:approved sources? are wrong|citations? are optional|don't use (?:the )?(?:approved )?sources?)\b|\bpretend\b.{0,100}\b(?:source id|approved|doctor|clinician|unrestricted|unfiltered)\b.{0,60}\b(?:bypass|validation|rule)?|\bpretend (?:that )?you(?:'re| are)\b.{0,60}\b(?:doctor|clinician|unrestricted|unfiltered)\b|\b(?:jailbreak|developer mode|do anything now|DAN mode)\b|\b(?:decode|translate)\b.{0,60}\b(?:base64|hex)\b.{0,60}\b(?:follow|execute|obey|instruction)s?\b/i;
const medicationAdjustmentPattern =
  /\b(?:should\s+i\s+)?(?:start|stop|skip|change|increase|decrease|adjust|double|halve|miss(?:ed)?|take)\b(?:\s+\w+){0,6}\s+(?:my\s+)?(?:medication|medicine|metformin|insulin|dose|dosage|prescription)\b|\b(?:medication|medicine|metformin|insulin|dose|dosage|prescription)\b.{0,100}\b(?:start|stop|skip|change|increase|decrease|adjust|double|halve|twice as much|make up|missed)\b|\b(?:forgot|missed)\b.{0,50}\b(?:medication|medicine|metformin|insulin|dose|dosage|prescription)\b|\b(?:how much|how many units|what dose|what dosage)\b.{0,60}\b(?:insulin|medication|medicine|metformin|prescription)\b/i;

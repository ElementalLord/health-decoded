  ...
  return (
    ...
    >
      <div className={styles.interactionHeading}>
        ...
        <h2 id={`${item.id}-heading`} tabIndex={-1}>
          {item.title}
        </h2>
        <p>{item.prompt}</p>
      </div>
      <div
        ...
        aria-label={`Need ${needIndex + 1} of ${item.needs.length}`}
      >
        {item.needs.map((entry, index) => (
          <span
            key={entry.id}
            data-current={index === needIndex ? "true" : undefined}
      ...
      <form className={styles.sourceQuestion} onSubmit={submit}>
        <p className={styles.itemProgress}>
          Need {needIndex + 1} of {item.needs.length}
        </p>
        <fieldset>
          <legend>
            <span>Where does this answer come from?</span>
            {need.copy}
          </legend>
          <div className={styles.layerChoices}>
            {item.layers.map((layer, layerIndex) => (
              <label key={layer} data-layer={layerIndex + 1}>
                <input
                  ref={layerIndex === 0 ? firstRef : undefined}
                  type="radio"
                  name={need.id}
                  value={layerIndex}
                  checked={selected === layerIndex}
                  onChange={(event) => {
                ...
                />
                <span>
                  <small>Source {layerIndex + 1}</small>
                  <strong>{layer}</strong>
                </span>
              </label>
            ))}
      ...
      {submitted ? (
        <CaregiverFeedback
          key={count}
          focusWhen
          heading={reviewed[need.id] ? "Source confirmed" : "This source needs review"}
          tone={reviewed[need.id] ? "supportive" : "warning"}
        >
      ...
      <div className={styles.itemNavigation}>
        <button type="button" disabled={needIndex === 0} onClick={() => moveTo(needIndex - 1)}>
          Previous need
        </button>
        <button
          type="button"
          disabled={!reviewed[need.id] || needIndex === item.needs.length - 1}
          onClick={() => moveTo(needIndex + 1)}
        >
          Next need
        </button>
      </div>
      {allReviewed ? (
        <div className={styles.completedLayerMap}>
          ...
          <ol>
            {item.layers.map((layer, layerIndex) => (
              <li key={layer}>
                ...
                <ul>
                  {item.needs
                    .filter((entry) => entry.preferred === layerIndex)
                    .map((entry) => (
                      <li key={entry.id}>{entry.copy}</li>
                    ))}
                </ul>
              </li>
            ))}

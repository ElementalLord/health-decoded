...
export function Module4KnowledgeCheck({
  ...
  function submit(event: React.FormEvent) {
    ...
    setKeyIdeaUnderstood(
      caregiverModule4.questions.every((entry) => nextAnswers[entry.id] === entry.preferredIndex),
    );
    setSubmitted(true);
  ...
  return (
    <section className={styles.knowledge} aria-labelledby="m4-knowledge-heading">
      ...
      <h2 id="m4-knowledge-heading" tabIndex={-1}>
        Check your understanding
      </h2>
      <p>One situation at a time. This review does not determine module completion.</p>
      <div className={styles.questionProgress}>
        {caregiverModule4.questions.map((entry, index) => (
          <span
            key={entry.id}
            data-current={index === questionIndex ? "true" : undefined}
            data-complete={reviewed[entry.id] ? "true" : undefined}
          >
            {reviewed[entry.id] ? "✓" : index + 1}
          </span>
        ))}
      </div>
      <form onSubmit={submit}>
        <fieldset id={question.id} className={styles.question}>
          <legend>
            <span>
              Situation {questionIndex + 1} of {caregiverModule4.questions.length}
            </span>
            {question.question}
          </legend>
          {question.choices.map((choice, choiceIndex) => (
            <label
              key={choice}
              data-needs-review={
                submitted && selected === choiceIndex && choiceIndex !== question.preferredIndex
                  ? "true"
                  : undefined
              }
            >
              <input
                ref={choiceIndex === 0 ? firstRef : undefined}
                type="radio"
                name={question.id}
                value={choiceIndex}
        ...
        <div className={styles.actions}>
          <button className={styles.primaryAction} type="submit" disabled={selected === undefined}>
            Review this response
          </button>
          {submitted && !reviewed[question.id] ? (
            <button
      ...
      {submitted ? (
        <CaregiverFeedback
          key={count}
          focusWhen
          heading={reviewed[question.id] ? "Key idea" : "This response needs review"}
          tone={reviewed[question.id] ? "supportive" : "warning"}
        >
          <p>{question.explanation}</p>
          {assisted[question.id] ? <p>Answer filled in after three attempts.</p> : null}
        </CaregiverFeedback>
      ) : null}
      <div className={styles.itemNavigation}>
        ...
        <button
          type="button"
          disabled={
            !reviewed[question.id] || questionIndex === caregiverModule4.questions.length - 1
          }
          onClick={() => moveTo(questionIndex + 1)}
        >
          Next situation
      ...
      {allReviewed ? (
        <p className={styles.interactionComplete}>{caregiverModule4.completion.keyIdea}</p>
      ) : null}
    </section>
  );
}

  ...
  return (
    <section id={section.id} className={styles.scenario} aria-labelledby={`${section.id}-heading`}>
      ...
      <figure className={styles.scenarioArt}>
        <Image
          src="/caregiver/module-4/unfinished-errand.png"
          alt="Celeste sits partway up an apartment stairwell beside her bag while Omar stands nearby with groceries, attentive and uncertain."
          width={1536}
          height={1024}
      ...
      <div className={styles.storyText}>
        {section.paragraphs.map((paragraph, index) => (
          <p
            key={paragraph}
            data-dialogue={index === 1 ? "omar" : index === 2 ? "celeste" : undefined}
          >
            {paragraph}
          </p>
        ))}
      ...
      <div className={styles.observationSplit}>
        <div>
          <p>What Omar can report</p>
          <ul>
            <li>Stopped halfway upstairs</li>
            <li>Sat down</li>
            <li>Answers more slowly than usual</li>
            <li>Said a plan is in her bag</li>
          </ul>
        </div>
        <div>
          <p>What Omar does not know</p>
          <ul>
            <li>What caused the change</li>
            <li>What a personal reading means</li>
            <li>Which improvised action would help</li>
          </ul>
        </div>
      </div>
    </section>
...
export function Module4Notice() {
  ...
  return (
    <section id={section.id} className={styles.notice} aria-labelledby={`${section.id}-heading`}>
      ...
      {section.paragraphs.map((paragraph, index) => (
        <p key={paragraph} className={index === 2 ? styles.readingBoundary : undefined}>
          {paragraph}
        </p>
      ))}
    </section>
    ...
    >
      <div className={styles.planCopy}>
        ...
        <ul className={styles.planContents} aria-label="What a clinician-created plan may contain">
          <li>Known signs</li>
          <li>Agreed supporter role</li>
          <li>Where instructions are kept</li>
          <li>Whom to contact</li>
        </ul>
      </div>
      <div className={styles.planBoundaries}>
        <section>
          <h3>The individualized layer</h3>
          <p>{caregiverModule4.safety.plan}</p>
        </section>
        <section className={styles.noReading}>
          <h3>Do not enter a glucose reading here</h3>
...
export function Module4HandoffNarrative() {
  ...
  return (
    ...
    >
      ...
      <p className={styles.doNotDelay}>
        <strong>{section.close}</strong> {caregiverModule4.safety.doNotDelay}
      </p>
      <details className={styles.professionalDisclosure}>
        <summary>Which human help does this module mean?</summary>
...
export function Module4UnsafeNarrative() {
  ...
  return (
    <section
      id={section.id}
      className={styles.unsafeNarrative}
      aria-labelledby={`${section.id}-heading`}
    >
...
export function Module4Misunderstanding() {
  ...
  return (
    ...
    >
      ...
      <div className={styles.delaySequence} aria-label="Repeated checking can preserve uncertainty">
        <span>One more reading</span>
        <span>One more search</span>
        <span>One more check</span>
...
export function Module4Scripts() {
  ...
  const renderScript = ([label, copy]: (typeof caregiverModule4.scripts)[number]) => (
    <div key={label}>
      <dt>{label}</dt>
      <dd>{copy}</dd>
    </div>
  ...
  return (
    <section className={styles.scripts} aria-labelledby="module-4-scripts-heading">
      ...
      <h2 id="module-4-scripts-heading" tabIndex={-1}>
        Language for the next layer
      </h2>
      <p>
        Use plain language that reports change, locates the plan, or names the need for human help.
      </p>
      <dl>{firstScripts.map(renderScript)}</dl>
      <details>
        <summary>See all useful phrases</summary>
        <dl>{moreScripts.map(renderScript)}</dl>
      </details>
    </section>
  );
}

...
export function ContextOrganizer() {
  ...
  const summary = item.choices
    .filter((entry) => entry.preferred && reviewed[entry.id] && answers[entry.id])
  ...
  function submit(event: React.FormEvent) {
    ...
    if (correct) {
      ...
      if (attempt >= 3) {
        ...
        setReviewed(nextReviewed);
        if (item.choices.every((entry) => nextReviewed[entry.id]))
  ...
  return (
    <section
      className={styles.contextOrganizer}
    ...
    >
      ...
      <div className={styles.contextLayout}>
        <form className={styles.statementCard} onSubmit={submit}>
          <p className={styles.itemProgress}>
            Statement {choiceIndex + 1} of {item.choices.length}
          </p>
        ...
        <aside className={styles.liveSummary} aria-label="Omar's factual summary">
          ...
          {summary.length ? (
            ...
            <p>Reviewed observations will appear here.</p>
          )}
        </aside>
      </div>
      {submitted ? (
        <CaregiverFeedback
          key={feedbackCount}
          focusWhen
          heading={reviewed[choice.id] ? "Statement reviewed" : "Look at the source of this claim"}
          tone={reviewed[choice.id] ? "supportive" : "warning"}
        >
          <p>{feedback}</p>
      ...
      <div className={styles.itemNavigation}>
        <button type="button" disabled={choiceIndex === 0} onClick={() => moveTo(choiceIndex - 1)}>
          Previous statement
        </button>
        <button
          type="button"
          disabled={!reviewed[choice.id] || choiceIndex === item.choices.length - 1}
          onClick={() => moveTo(choiceIndex + 1)}
        >
          Next statement
        </button>
      </div>

...
export function UnsafeImprovisationReview() {
  ...
  function submit(event: React.FormEvent) {
    ...
    if (selected === action.unsafe) {
      ...
      if (attempt >= 3) {
        ...
        setReviewed(nextReviewed);
        if (item.actions.every((entry) => nextReviewed[entry.id]))
  ...
  return (
    <section
      className={styles.improvisationReview}
    ...
    >
      ...
      <form className={styles.actionQuestion} onSubmit={submit}>
        <p className={styles.itemProgress}>
          Action {actionIndex + 1} of {item.actions.length}
        </p>
        <fieldset>
          <legend>
            <span>Should Omar invent this action from this module?</span>
            {action.copy}
          </legend>
          <label>
            <input
          ...
          <label>
            ...
            />
            <span>This uses an existing plan or human guidance</span>
          </label>
        </fieldset>
        ...
        <CaregiverFeedback
          ...
          heading={
            reviewed[action.id]
              ...
              : "Look at where this action comes from"
          }
          tone={reviewed[action.id] ? "supportive" : "warning"}
        >
          <p>{action.feedback}</p>
      ...
      <div className={styles.itemNavigation}>
        <button type="button" disabled={actionIndex === 0} onClick={() => moveTo(actionIndex - 1)}>
          Previous action
        </button>
        <button
          type="button"
          disabled={!reviewed[action.id] || actionIndex === item.actions.length - 1}
          onClick={() => moveTo(actionIndex + 1)}
        >
          Next action
        </button>
      </div>

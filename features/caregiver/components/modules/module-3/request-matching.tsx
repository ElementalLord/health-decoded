...
export function RequestMatching() {
  ...
  function checkCurrent(event: React.FormEvent) {
    ...
    if (matches[pair.id] === pair.id) {
      ...
      setMessage("fit");
      if (interaction.pairs.every((item) => nextReviewed[item.id]))
  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-label={`Request ${pairIndex + 1} of ${interaction.pairs.length}`}
      >
        {interaction.pairs.map((item, index) => (
          <span
            key={item.id}
            data-current={index === pairIndex ? "true" : undefined}
            data-complete={reviewed[item.id] ? "true" : undefined}
          >
            {reviewed[item.id] ? "✓" : index + 1}
          </span>
        ))}
      </div>
      <form onSubmit={checkCurrent} className={styles.matchCard}>
        <p className={styles.itemProgress}>
          Request {pairIndex + 1} of {interaction.pairs.length}
        </p>
        ...
        <button className={styles.primaryAction} type="submit" disabled={!matches[pair.id]}>
          Check this match
        </button>
      </form>
      {message ? (
        <CaregiverFeedback
          focusWhen
          heading={message === "review" ? "Try this request again" : "This offer fits"}
        >
          <p>
            {message === "review" ? interaction.feedback.adjacent : interaction.feedback.preferred}
          </p>
          {assisted[pair.id] ? (
            <p className={styles.answerAssist}>
              The fitting offer was filled in after three attempts so you can continue.
            </p>
          ) : null}
        </CaregiverFeedback>
      ) : null}
      <div className={styles.itemNavigation}>
        <button type="button" disabled={pairIndex === 0} onClick={() => moveTo(pairIndex - 1)}>
          Previous request
        </button>
        <button
          type="button"
          disabled={!reviewed[pair.id] || pairIndex === interaction.pairs.length - 1}
          onClick={() => moveTo(pairIndex + 1)}
        >
          Next request
        </button>
      </div>
      {allReviewed ? (
        <p className={styles.coreComplete}>
          <strong>All four requests matched.</strong> {interaction.learningPoint}
        </p>
      ) : null}
    </section>
  );

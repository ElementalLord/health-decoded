...
const stages = [
  {
    ...
    headingId: "caregiver-module-3-heading",
  },
  ...
  { id: "normal", group: "Balance", label: "Preserve normal life", headingId: "CG-M3-S06-heading" },
  {
    ...
    headingId: "module-3-further-reading-heading",
  },
  {
    id: "check",
    group: "Review",
    label: "Check your understanding",
    headingId: "module-3-check-heading",
  },
    ...
    >
      <header className={styles.lessonHeader}>
        ...
        <div className={styles.progressCopy}>
          ...
          <ProgressBar
            ...
            label={`Module 3 progress: part ${stageIndex + 1} of ${stages.length}`}
      ...
      <div className={styles.stageViewport}>
        <div className={styles.stage} data-stage="opening" hidden={stageIndex !== 0}>
          <Module3Orientation onBegin={() => goToStage(1)} />
        </div>
        <div className={styles.stage} data-stage="scenario" hidden={stageIndex !== 1}>
          <Module3Scenario />
        </div>
        <div className={styles.stage} data-stage="meals" hidden={stageIndex !== 2}>
          <Module3MealsNarrative />
        </div>
        <div className={styles.stage} data-stage="plan" hidden={stageIndex !== 3}>
          <SharedPlanningWorkspace />
        </div>
        <div className={styles.stage} data-stage="specific" hidden={stageIndex !== 4}>
          <ModuleVisibilityMarker onViewed={markCentralIdeaReached}>
            <Module3SpecificNarrative />
          </ModuleVisibilityMarker>
        </div>
        <div className={styles.stage} data-stage="match" hidden={stageIndex !== 5}>
          <RequestMatching />
        </div>
        <div className={styles.stage} data-stage="menu" hidden={stageIndex !== 6}>
          <Module3ChangesNarrative />
          <SupportMenu />
        </div>
        <div className={styles.stage} data-stage="routines" hidden={stageIndex !== 7}>
          <RoutineComparison />
        </div>
        <div className={styles.stage} data-stage="normal" hidden={stageIndex !== 8}>
          <Module3NormalLifeNarrative />
          <Module3MisunderstandingNarrative />
        </div>
        ...
        <div className={styles.stage} data-stage="check" hidden={stageIndex !== 10}>
          <Module3KnowledgeCheck
            onReviewSection={(sectionId) => goToStage(sectionStageIndex[sectionId] ?? 4)}
          />
        </div>
        <div className={styles.stage} data-stage="takeaway" hidden={stageIndex !== 11}>
          <Module3Takeaway />
          <Module3Reflection />
          <Module3Completion onReview={() => goToStage(4)} />
        </div>
      </div>

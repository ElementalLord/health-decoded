...
export function Module3KnowledgeCheck({
  ...
  function submit(event: React.FormEvent) {
    ...
    setKeyIdeaUnderstood(
      caregiverModule3.questions.every((item) => nextAnswers[item.id] === item.preferredIndex),
    );
    setSubmitted(true);
    ...
    <section className={styles.knowledgeCheck} aria-labelledby="module-3-check-heading">
      ...
      <form onSubmit={submit}>
        <fieldset id={question.id} className={styles.question}>
          <legend>
            <span>
              Situation {questionIndex + 1} of {caregiverModule3.questions.length}
            </span>
            {question.question}
          </legend>
        ...
        <div className={styles.interactionActions}>
          ...
          {submitted && !isPreferred && !assistedAnswers[question.id] ? (
            <button
      ...
      {submitted ? (
        <CaregiverFeedback
          key={submissionCount}
          focusWhen
          heading={reviewed[question.id] ? "Key idea" : "Look once more"}
        >
      ...
      <div className={styles.itemNavigation}>
        ...
        <button
          type="button"
          disabled={
            !reviewed[question.id] || questionIndex === caregiverModule3.questions.length - 1
          }
          onClick={() => moveTo(questionIndex + 1)}
        >
          Next situation
      ...
      {allReviewed ? (
        <p className={styles.coreComplete}>{caregiverModule3.completion.keyIdea}</p>
      ) : null}
    </section>
  );
}

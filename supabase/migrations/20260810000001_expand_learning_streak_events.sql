-- Controlled categories only. Event payloads never include learning or health content.
alter table public.user_learning_activity_days
  drop constraint user_learning_activity_days_event_valid;

alter table public.user_learning_activity_days
  add constraint user_learning_activity_days_event_valid check (
    qualifying_event_type = any (array[
      ...
      'ai_learning_exchange_completed', 'clinician_questions_prepared'
    ]::pg_catalog.text[])
  );
...
begin
  ...
  select streak.current_streak, streak.longest_streak, streak.freeze_balance, streak.last_qualified_date into v_current, v_longest, v_freezes, v_previous_date from public.user_learning_streaks as streak where streak.user_id = v_user_id for update;

-- Keep every verified learning surface on the same daily, idempotent streak path.
-- The migration is additive: existing activity days and streak totals are preserved.
...
alter table public.user_learning_activity_days
  add constraint user_learning_activity_days_event_valid check (
    qualifying_event_type = any (array[
      ...
      'myth_sources_reviewed', 'appointment_priorities_completed',
      'appointment_questions_completed', 'appointment_summary_completed',
      'appointment_summary_exported', 'caregiver_module_completed',
...
begin
  ...
  update public.user_learning_streaks as streak set
    ...
    last_qualified_date = case
      when v_inserted > 0 then v_activity_date else streak.last_qualified_date
    end,
    last_continuity_date = v_previous_date,
    timezone = v_timezone,
